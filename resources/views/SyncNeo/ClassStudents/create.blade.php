...
@section('title', 'Neo Class Students sync create')
@section('title-active', 'Cypherlearning Neo Class Students sync')
...
@section('content')
<form method="POST" id="sync-form" action="{{ route('syncclassstudents.store') }}" >
    @csrf
...
document.getElementById('confirm-sync').addEventListener('click', function () {
    Swal.fire({
        title: '¿Estás seguro?',
        text: "Esto sincronizará los students class desde NeoAPI.",
        icon: 'warning',
