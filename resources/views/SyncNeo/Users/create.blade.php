...
@section('title', 'Neo Users sync create')
@section('title-active', 'Cypherlearning Neo Users sync')
...
@section('content')
<form method="POST" id="sync-form" action="{{ route('syncusers.store') }}" >
    @csrf
...
document.getElementById('confirm-sync').addEventListener('click', function () {
    Swal.fire({
        title: '¿Estás seguro?',
        text: "Esto sincronizará los usuarios desde NeoAPI.",
        icon: 'warning',
