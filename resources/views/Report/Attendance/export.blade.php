...
@section('title', 'Neo report attendance users')
@section('title-active', 'Cypherlearning Neo report attendance users')
...
@section('content')
<form method="POST" id="sync-form" action="{{ route('attendanceusers.export') }}" >
    @csrf
    <div class="nav-tabs-top">
        <ul class="nav nav-tabs">
            <li class="nav-item">
                <a class="nav-link active" data-toggle="tab" href="#user-edit-account">Neo Report</a>
            </li>
...
<script>
    $(document).ready(function () {
        $('#neo_tenant_id').on('change', function () {
            ...
            if (tenantId) {
                $.ajax({
                    url: '../organizationsattendance/' + tenantId,
                    type: 'GET',
                    dataType: 'json',
...
document.getElementById('confirm-sync').addEventListener('click', function () {
    Swal.fire({
        title: '¿Estás seguro?',
        text: "Esto generara el reporte de attendance.",
        icon: 'warning',
