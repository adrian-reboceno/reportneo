...
@section('title-active', 'Manager Profile')
            ...
            <div class="card-body">
                <div class="row align-items-center m-l-0">
                    ...
                    <div class="col-sm-6 text-right">
                        @can('user-create')   
                            <a class="btn btn-success btn-sm mb-3 btn-round" href="{{route('neoprofiles.create')}}"><i class="feather icon-plus"></i> Add Profile</a>
                        @endcan
                    </div>
                </div>
                <div >
                    <table id="datatables" class="datatables-demo table table-striped table-bordered dataTable no-footer" role="grid" aria-describedby="report-table_info" style="width:100%">
                        ...
                        <tbody>
                            ...
                            <tr>
                                <td>{{$NeoProfile->id}}</td>                                                   
                                <td>{{$NeoProfile->profile_name}}</td>                                 
                                <td>{{$NeoProfile->created_at}}</td>   
                                <td>{{$NeoProfile->updated_at}}</td>                         
                                <td>
                                    <div class="btn-group">
                                        <button type="button" class="btn btn-secondary dropdown-toggle" data-toggle="dropdown">Acction</button>
                                        <div class="dropdown-menu">
                                            @can('user-create')   
                                                <li><a href="{{route('neoprofiles.show', $NeoProfile->id)}}" class="dropdown-item"><i class="sidenav-icon feather icon-eye align-bottom me-2 text-muted"></i> View</a></li>
                                            @endcan
                                            @can('user-create')   
                                                <li><a href="{{route('neoprofiles.edit', $NeoProfile->id)}}" class="dropdown-item edit-item-btn"><i class="sidenav-icon feather icon-edit-2 align-bottom me-2 text-muted"></i> Edit</a></li>
                                            @endcan
                                            @can('user-create')                                              
                                                <li>                                               
                                                    <form action="{{ route('neoprofiles.destroy', $NeoProfile->id) }}" method="POST">
                                                        @csrf
                                                        @method('DELETE')
